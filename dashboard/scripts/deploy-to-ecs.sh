...
echo "=== 3. Rsync static ==="
eval "$RSYNC .next/static/ ${DEST}.next/static/"

echo "=== 4. Fix platform-specific native modules ==="
eval "$SSH \"$ECS_HOST\" 'cd /opt/dashboard && if [ ! -f node_modules/better-sqlite3/build/Release/better_sqlite3.node ]; then npm pack better-sqlite3 && tar xzf better-sqlite3-*.tgz && rm -rf node_modules/better-sqlite3 && mv package node_modules/better-sqlite3 && cd node_modules/better-sqlite3 && npx node-gyp rebuild && rm -f /opt/dashboard/better-sqlite3-*.tgz; fi'"

echo "=== 5. PM2 reload ==="
eval "$SSH \"$ECS_HOST\" 'pm2 reload dashboard --update-env && pm2 status'"
